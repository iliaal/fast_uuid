...
Generation stays on the pure-C fast path; supplying a custom `RandomGeneratorInterface` / `TimeGeneratorInterface` / `NodeProviderInterface` intentionally routes off it where needed (ramsey behaviour) so application-supplied generators win. Custom node providers feed `uuid1()`, `uuid2()`, `uuid6()`, and `fromDateTime()` when no explicit node is passed. The compat `Uuid::uuid7()` facade also accepts a unix-millisecond `int`, matching the core object API. Decoded UUID objects retain the factory's active codec for strings, bytes, and serialization, and every derived identity form follows `toString()` the way ramsey defines it: `getUrn()` is `'urn:uuid:' . toString()`, `getHex()` is `toString()` without the hyphens, while `getInteger()` always reads the network-order core. `equals()` takes any value and returns false for non-UUID input instead of throwing; `compareTo()` accepts UUID objects only and orders by the network-order bytes regardless of codec, throwing `InvalidArgumentException` otherwise. Under the default codec that is the canonical form, so none of this changes unless you set a codec. `GuidStringCodec` swaps the first three fields in `encodeBinary()` / `decodeBytes()` only: a GUID is mixed-endian in its byte array, while its text stays the plain RFC string, which is why `.NET`'s `Guid.ToString()` and `Guid.ToByteArray()` disagree. Pass GUID-ordered bytes to `fromBytes()` under that codec and canonical text to `fromString()`; `Guid\Guid::getBytes()` exposes the same mixed-endian order. `OrderedTimeCodec` and the COMB codecs change their documented binary storage order, and the COMB codecs also reshape the text, so their `getHex()` follows it. Migration from `ramsey/uuid` is largely a `use` swap from `Ramsey\Uuid\Uuid` to `FastUuid\Compat\Uuid`. The compat package has no external dependencies beyond the extension itself.
