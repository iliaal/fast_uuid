--TEST--
Input validation: timestamp range (v1/v7), node/clockSeq bounds, uuid2 domain, fromHexadecimal Stringable, userland namespace
--EXTENSIONS--
fast_uuid
--FILE--
<?php
declare(strict_types=1);
use FastUuid\Uuid;
use FastUuid\Exception\InvalidArgumentException as IAE;

function throws(callable $fn, string $class = IAE::class): bool {
    try { $fn(); return false; } catch (\Throwable $e) { return $e instanceof $class; }
}
$utc = new DateTimeZone('UTC');

// One second past the 48-bit ms ceiling (intdiv(2**48, 1000) + 1). A literal,
// not 1 << 48, since that shift is 0 on 32-bit PHP; DateTime holds the large
// instant via its 64-bit internal time.
$overV7 = new DateTimeImmutable('@281474976711');
var_dump(throws(fn() => Uuid::uuid7($overV7)));
var_dump(throws(fn() => Uuid::uuid7(new DateTimeImmutable('@-1'))));
var_dump(Uuid::uuid7(new DateTimeImmutable('2023-01-01 00:00:00.5', $utc))->getVersion() === 7);

var_dump(throws(fn() => Uuid::fromDateTime(new DateTimeImmutable('6000-01-01', $utc)))); // > ~year 5236 throws
var_dump(throws(fn() => Uuid::fromDateTime(new DateTimeImmutable('1500-01-01', $utc)))); // < 1582 throws
var_dump(Uuid::fromDateTime(new DateTimeImmutable('1969-12-31 23:59:59', $utc))->getVersion() === 1);
var_dump(Uuid::fromDateTime(new DateTimeImmutable('2020-06-15 12:00:00.5', $utc))->getVersion() === 1);

var_dump(throws(fn() => Uuid::uuid1(-1)));
// 48-bit node literals exceed PHP_INT_MAX on 32-bit PHP (they parse as float),
// so test the numeric-overflow rejection and max node via int on 64-bit and
// via string on 32-bit.
if (PHP_INT_SIZE >= 8) {
    var_dump(throws(fn() => Uuid::uuid1(2 ** 48)));
    var_dump(substr(Uuid::uuid1(0xffffffffffff)->getHex(), 20) === 'ffffffffffff');
} else {
    var_dump(throws(fn() => Uuid::uuid1('zzzzzzzzzzzz')));
    var_dump(substr(Uuid::uuid1('ffffffffffff')->getHex(), 20) === 'ffffffffffff');
}
var_dump(throws(fn() => Uuid::uuid1('0123456789ab', 0x4000)));
var_dump(Uuid::uuid1('0123456789ab', 0x3fff)->getVersion() === 1);

var_dump(throws(fn() => Uuid::uuid2(2)));
var_dump(Uuid::uuid2(2, 100)->getVersion() === 2);
if (PHP_OS_FAMILY === 'Windows') {
    // No POSIX uid/gid to auto-fill from: PERSON/GROUP without an id throw.
    var_dump(throws(fn() => Uuid::uuid2(0)));
    var_dump(throws(fn() => Uuid::uuid2(1)));
} else {
    var_dump(Uuid::uuid2(0)->getVersion() === 2);
    var_dump(Uuid::uuid2(1)->getVersion() === 2);
}

$thrower = new class { public function __toString(): string { throw new \RuntimeException('boom'); } };
var_dump(throws(fn() => Uuid::fromHexadecimal($thrower), \RuntimeException::class));
var_dump(throws(fn() => Uuid::fromHexadecimal(new stdClass()), \TypeError::class));
$hexObj = new class implements \Stringable { public function __toString(): string { return '0a1b2c3d4e5f60718293a4b5c6d7e8f9'; } };
var_dump(Uuid::fromHexadecimal($hexObj)->getHex() === '0a1b2c3d4e5f60718293a4b5c6d7e8f9');

$ns = new class implements \FastUuid\UuidInterface {
    public function __toString(): string { return '6ba7b810-9dad-11d1-80b4-00c04fd430c8'; }
    public function jsonSerialize(): mixed { return (string) $this; }
};
var_dump(Uuid::uuid5($ns, 'x')->getVersion() === 5);
var_dump(Uuid::uuid3($ns, 'x')->getVersion() === 3);
var_dump(Uuid::uuid5($ns, 'x')->equals(Uuid::uuid5('6ba7b810-9dad-11d1-80b4-00c04fd430c8', 'x')));
?>
--EXPECT--
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
bool(true)
